				...
				<div class="row">
					...
					<div class="col-xs-12 col-sm-12 col-md-12"> <!-- Feature 1 -->
						<div class="single-feature-item wow fadeIn text-left mt-0">
							...
							<?php 
							$rslt=mysqli_query($con,"SELECT * from tbl_insurance order by id asc");
								while($row=mysqli_fetch_assoc($rslt))
								{
							?>
							<a href="<?php echo $absolutePath;?>insurance-detail.php?id=<?php echo $row['id'];?>"><div class="col-md-3"><div style='height: 161px;overflow: hidden;'>
								<?php if(!empty($row["logo"])){?><img src="<?php echo $absolutePath;?>uploads/images/insurance/<?php echo $row["logo"]; ?>" alt="<?php echo $row["insurance"]; ?>" style="width: 100%"/><?php }?></div>
								<p><?php echo substr($row["insurance"],0,25) ?>...</p>
							</div></a>
								<?php }?>
						</div>
					</div>
