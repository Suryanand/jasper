    ...
    <div class="Breadcrumbs page-header-img-1"> <!-- Slider Image -->
        ...
        <div class="slider-item-table-cell">
            ...
            <div class="row">
                ...
                <h2 class="wow fadeInDown">Fitness Trainers</h2>
                <ol class="breadcrumb">
                          <li><a href="<?php echo $absolutePath;?>">Home</a></li>
                          <li class="active">Fitness Trainers</li>
                        </ol>
              </div>
                    </div>
          </div>
...
<?php
$rslt=mysqli_query($con,"SELECT * FROM tbl_fitness_specialty where activeStatus='1' order by Name DESC");
while($row=mysqli_fetch_array($rslt))
{
?>
<form method="post" action="<?php echo $absolutePath;?>trainer-department.php">
...
<br clear="all"/> <br clear="all"/>        
              <div class="col-md-12">
        <div class="tab">
                  <button class="tablinks" onclick="openCity(event, '1')" id="defaultOpen">Find a Fitness Trainer</button>
        ...
        <div id="1" class="tabcontent" style='height: 80px;     padding: 23px 30px !important;'>
                  <form action="search-trainer.php" method="get">
...
<?php 
$rslt=mysqli_query($con,"select distinct area from tbl_trainers where type=0 order by area asc limit 0,5");
while($row=mysqli_fetch_assoc($rslt)){ ?>
...
<?php }?>
<?php 
$rslt=mysqli_query($con,"select distinct area from tbl_trainers where type=0 order by area asc limit 5,1000");
while($row=mysqli_fetch_assoc($rslt)){ ?>
...
<?php 
$rslt=mysqli_query($con,"select distinct nationality from tbl_trainers where type=0 order by nationality asc limit 0,5");
while($row=mysqli_fetch_assoc($rslt)){?>
<label class="filter-label">
<input type="checkbox" class="nationality" value="<?php echo $row["nationality"];?>"><?php echo $row["nationality"];?></label>
<?php }?>
<?php 
$rslt=mysqli_query($con,"select distinct nationality from tbl_trainers where type=0 order by nationality asc limit 5,1000");
while($row=mysqli_fetch_assoc($rslt)){?>
...
<label for="mycheckbox2" class="showmore2" style="float: right;"><i style="color: #727272;font-size: 12px !important;">more...</i></label>
  ...
  <div class="contact-info wow fadeIn">
                  ...
                  <?php 
						$rslt=mysqli_query($con,"select * from tbl_insurance order by insurance asc limit 0,5");
						while($row=mysqli_fetch_assoc($rslt))
						{
						?>
           ...
           <?php echo substr($row["insurance"],0,20);?>...</label>
                  <?php }?>
                         <?php 
						$rslt=mysqli_query($con,"select * from tbl_insurance order by insurance asc limit 5,1000");
						while($row=mysqli_fetch_assoc($rslt))
						{
						?>
        ...
        <div class="doc_class" >
                  ...
                  <a href="<?php echo $absolutePath;?>trainers/<?php echo $row["urlName"];?>">
