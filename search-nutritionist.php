...
<?php if($specialty!=''){?>  					
					...
					if(!empty($location))
						...
						$qry="";
					$rslt=mysqli_query($con,"select * from tbl_trainers where activeStatus=1 and type=1 and (specialized like '%".$specialty."%' OR qualification like '%".$specialty."%'".$qry.")");
...
//					else
//						$qry="";
					$rslt=mysqli_query($con,"select * from tbl_trainers where activeStatus=1 and type=1 and (fullName like '%".$lname."%' OR email like '%".$lname."%')");
					while($row=mysqli_fetch_assoc($rslt))
					{
						if(!empty($row["image"]))
						{
