    ...
    <div class="Breadcrumbs page-header-img-1"> <!-- Slider Image -->
        ...
        <div class="slider-item-table-cell">
            ...
            <div class="row">
                ...
                <h2 class="wow fadeInDown">Nutritionists</h2>
                <ol class="breadcrumb">
                          <li><a href="<?php echo $absolutePath;?>">Home</a></li>
                          <li class="active">Nutritionists</li>
                        </ol>
              </div>
                    </div>
          </div>
...
{
?>
<form method="post" action="<?php echo $absolutePath;?>nutritionist-department.php">
...
<br clear="all"/> <br clear="all"/>        
        ...
        <div id="1" class="tabcontent" style='height: 80px;     padding: 23px 30px !important;'>
                  <form action="search-nutritionist.php" method="get">
...
<?php 
$rslt=mysqli_query($con,"select distinct area from tbl_trainers where type=1 order by area asc limit 0,5");
while($row=mysqli_fetch_assoc($rslt)){ ?>
...
<?php }?>
<?php 
$rslt=mysqli_query($con,"select distinct area from tbl_trainers where type=1 order by area asc limit 5,1000");
while($row=mysqli_fetch_assoc($rslt)){ ?>
...
<?php 
$rslt=mysqli_query($con,"select distinct nationality from tbl_trainers where type=1 order by nationality asc limit 0,5");
while($row=mysqli_fetch_assoc($rslt)){?>
<label class="filter-label">
<input type="checkbox" class="nationality" value="<?php echo $row["nationality"];?>"><?php echo $row["nationality"];?></label>
<?php }?>
<?php 
$rslt=mysqli_query($con,"select distinct nationality from tbl_trainers where type=1 order by nationality asc limit 5,1000");
while($row=mysqli_fetch_assoc($rslt)){?>
...
<label for="mycheckbox2" class="showmore2" style="float: right;"><i style="color: #727272;font-size: 12px !important;">more...</i></label>
  ...
  <div class="contact-info wow fadeIn">
        ...
        <?php 
					$rslt=mysqli_query($con,"select * from tbl_trainers where activeStatus=1 and type=1 order by fullName asc");
					while($row=mysqli_fetch_assoc($rslt))
					{
						if(!empty($row["image"]))
						{
