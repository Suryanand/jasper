...
if(isset($_POST["doctor"]))
{
	...
	{
		$spec=$_POST["spec"];	
		$speclist=implode(',',$spec);
		$qry3=" and find_in_set(specialized,'$speclist')";
	}
	$rslt=mysqli_query($con,"select * from tbl_doctors where activeStatus=1".$qry1.$qry2.$qry3);
...
//						$row5=mysqli_fetch_assoc($rslt5);
          ...
          ?>
            <p style="font-size:14px; line-height:14px;"><i style="margin-right:10px" class="fa fa-crosshairs" aria-hidden="true"></i><?php echo substr($shortprofile,0,65);?> <a href="<?php echo $absolutePath;?>doctors/<?php echo $row["urlName"];?>">See more...</a></p>
          </div>
                  <hr>
                </div>
                  <?php
...
if(isset($_POST["hospital"]))
{
        ...
        if(isset($_POST["spec"]))
	{
		$spec=$_POST["spec"];
		$speclist=implode(',',$spec);
		$qry2=" and find_in_set(specialty,'$speclist')";
	}
	$rslt=mysqli_query($con,"select * from tbl_gymnasium where activeStatus=1 and companyType='$companyType'".$qry1.$qry2);
					while($row=mysqli_fetch_assoc($rslt))
					{
