...
if(isset($_POST["doctor"]))
{
	...
	if(isset($_POST["loc"]))
	{
		$loc=$_POST["loc"];
		$loclist=implode(',',$loc);
		$qry2=" and find_in_set(area,'$loclist')";
	}
        if(isset($_POST["nat"]))
	{
		$nat=$_POST["nat"];
		$natlist=implode(',',$nat);
		$qry2=" and find_in_set(nationality,'$natlist')";
	}
	if(isset($_POST["spec"]))
	{
		$spec=$_POST["spec"];	
	...
	$rslt=mysqli_query($con,"select * from tbl_trainers where type=3 and activeStatus=1".$qry1.$qry2.$qry3.$qry4);
...
if(isset($_POST["hospital"]))
{
	...
	if(isset($_POST["loc"]))
	{
		$loc=$_POST["loc"];
		$loclist=implode(',',$loc);
		$qry1=" and find_in_set(area,'$loclist')";
                $qry2=" and find_in_set(specialty,'$speclist')";
	}
	$rslt=mysqli_query($con,"select * from tbl_firm where activeStatus=1 and companyType='$companyType'".$qry1.$qry2);
					while($row=mysqli_fetch_assoc($rslt))
					{
