...
<!--- search articles------>                                                                            
<?php 
$rslt=mysqli_query($con,"select * from tbl_articles where activeStatus=1 and (title like '%".$key."%' OR description like '%".$key."%')");
while($row=mysqli_fetch_assoc($rslt))
{
$image=$absolutePath."uploads/images/blog/".$row["image"];
$crop_str='...';
$n_chars=200;
...
<div class="col-md-10">
<a href="<?php echo $absolutePath."article/".$row["urlName"];?>"><h3><?php echo $row["title"];?></h3></a>
<?php echo $buff;?>
</div>
<br clear="all"/>
<hr>
...
<!--- search pages------> 
<?php 
$rslt=mysqli_query($con,"select * from tbl_pages where activeStatus=1 and (pageTitle like '%".$key."%' OR description like '%".$key."%')");
while($row=mysqli_fetch_assoc($rslt))
{
$image=$absolutePath."uploads/images/".$row["image"];
?>
<div class="col-md-3" style="float:right">
<img src="<?php echo $image;?>" style="width:100%;height:100%">
</div>
<div class="col-md-9">
<a href="#"><h3><?php echo $row["pageTitle"];?></h3></a>
