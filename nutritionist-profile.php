...
$rslt=mysqli_query($con,"select * from tbl_trainers where urlName='".$urlName."' and type='1'");
$row=mysqli_fetch_assoc($rslt);
	$address = $row["address"];
	$doctorId=$row["id"];
	$area = $row["area"];
	...
	<p>&nbsp;</p>
	';
	mysqli_query($con,"insert into tbl_nutritionist_appointments(fullName,email,contactNo,appointmentDate,doctorId,subject,registrationDate) values('$fromName','$fromEmail','$phone','$appointmentDate','$doctorId','$comment',NOW())");
	/* body of mail ends*/
	$response="Your booking is successfully completed";
	require "includes/mail.php";
	echo "<script>location.href = '".$absolutePath."nutritionist/$urlName'</script>;";
...
$ip=$_POST['Ip'];
$time=$_POST['time'];
mysqli_query($con,"insert into tbl_nutritionist_comments(userId,doctorId,Ip,Message,updatedOn,time) values('$uid','$did','$ip','$comment',NOW(),'$time')");
?>
							...
							<div class="container">
								<div class="row">
									<div class="col-md-12 slider-content"> <!-- Slider Text & Button -->
										<h2 class="wow fadeInDown">Nutritionist Profile</h2>
										<ol class="breadcrumb">
                                            <li><a href="<?php echo $absolutePath;?>">Home</a></li>                                            
                                            <li class="active">Nutritionist Profile</li>
                                        </ol>
									</div>
								</div>
							</div>
...
$docid=$_POST['docId'];
$ip=$_POST['Ip'];
mysqli_query($con,"insert into tbl_nutritionist_review (docId,Ip,Name,Email,Comment,updatedOn)values('".$docid."','".$ip."','".$usrname."','".$usremail."','".$usrcomment."',NOW())") ;
?>
<script type="text/javascript">alert("Thank you for your valuable feedback.")</script>                
<?php
}
